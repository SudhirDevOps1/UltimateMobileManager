...
<body>
    ...
    <script>
        ...
        function navUp() {
            ...
            if (path === '/' || path.match(/^[A-Za-z]:\/$/)) return; // Root check
        ...
        if self.path.startswith('/api/files'):
            ...
            if mode == 'mobile':
                ...
                try:
                    ...
                    # Simple security check to prevent going too high? 
                    # For now user asked for full access "pura safe aur secrity full ho" usually means prevent hackers, not owner.
                    # Since PIN is required, owner is authenticated.
                    ...
                    if os.path.isdir(path):
                        for name in os.listdir(path):
                            try:
                                ...
                                items.append({'name': name, 'is_dir': is_dir, 'size': size, 'full_path': full.replace('\\', '/')})
                            except: pass
                except Exception as e:
                    pass
