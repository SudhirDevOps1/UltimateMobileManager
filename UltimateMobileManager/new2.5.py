...
<head>
    ...
    <style>
        ...
        .container { max-width: 800px; margin: 0 auto; padding: 2rem; width: 100%; box-sizing: border-box; }
        .card { background: var(--surface); border-radius: 1rem; padding: 2rem; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); margin-bottom: 2rem; border: 1px solid #334155; }
        h1 { margin: 0 0 2rem 0; font-weight: 300; letter-spacing: -1px; }
        ...
        button { background: var(--primary); border: none; padding: 1rem 2rem; color: white; border-radius: 0.5rem; font-size: 1rem; cursor: pointer; transition: 0.2s; width: 100%; font-weight: bold; }
        ...
        #file-list { list-style: none; padding: 0; margin-top: 2rem; }
        .file-item { display: flex; align-items: center; padding: 1rem; border-bottom: 1px solid #334155; transition: 0.2s; }
        .file-item:hover { background: #334155; }
        .file-icon { margin-right: 1rem; font-size: 1.25rem; }
        .file-name { flex: 1; }
        .file-meta { color: #94a3b8; font-size: 0.875rem; }
        .logout { position: absolute; top: 1rem; right: 1rem; background: transparent; border: 1px solid #ef4444; color: #ef4444; width: auto; padding: 0.5rem 1rem; font-size: 0.875rem; }
...
<body>
    ...
    <div id="dashboard-view" class="container hidden">
        <button class="logout" onclick="logout()">Exit Session</button>
        <div class="card">
        ...
        <div class="card">
            ...
            <div style="display: flex; gap: 0.5rem; margin-bottom: 1rem;">
                <button style="width: auto; padding: 0.5rem 1rem; background: #334155;" onclick="loadFiles(currentPath + '/..')">⬆ Up</button>
                <div id="current-path" style="background: #0f172a; padding: 0.5rem; border-radius: 0.25rem; flex: 1; display: flex; align-items: center; font-family: monospace;">/sdcard</div>
            </div>
            <ul id="file-list">
                <!-- Populated by JS -->
            </ul>
    ...
    <script>
        ...
        async function loadFiles(path) {
            const res = await fetch('/api/files?path=' + encodeURIComponent(path), { headers: {'Authorization': token} });
            if (res.status === 401) return logout();
            const data = await res.json();
            ...
            data.items.forEach(item => {
                ...
                `;
                if (item.is_dir) {
                    li.onclick = () => loadFiles(item.full_path);
...
"""
    ...
    def __init__(self, adb_manager):
        ...
        self.port = 0
        self.pin = secrets.token_hex(2).upper()  # 4 char HEX
...
class UltimateMobileManager:
    ...
    def _create_menu(self):
        ...
        device_menu.add_command(label="Enable Wi-Fi Mode", command=self._enable_wifi_mode)
        ...
        tools_menu.add_command(label="Start Web Access", command=self._start_web_server)
        
        # Help Menu
        help_menu = Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Help", menu=help_menu)
        help_menu.add_command(label="About", command=lambda: messagebox.showinfo("About", "Ultimate Mobile Manager v5.1\nBy Sudhir Kumar"))
    ...
    def _create_header(self, parent):
        ...
        Button(header, text="[Cam] Screenshot", command=self._take_screenshot, **btn_cfg).pack(side=RIGHT, padx=3, pady=10)
        Button(header, text="[TV] Mirror", command=self._start_scrcpy, **btn_cfg).pack(side=RIGHT, padx=3, pady=10)
        Button(header, text="[Ref] Refresh", command=self._refresh_all, **btn_cfg).pack(side=RIGHT, padx=3, pady=10)
