...
class Config:
    ...
    # Standard paths for legacy compatibility
    ADB_PATH = PLATFORM_TOOLS / "adb.exe"
    SCRCPY_PATH = SCRIPT_DIR / "scrcpy" / "scrcpy.exe"
    
    @staticmethod
    def find_executable(name):
        """Deep search for an executable"""
        # 1. Check in specific folders
        folders = [
            Config.SCRIPT_DIR,
            Config.SCRIPT_DIR / "platform-tools",
...
class ADBManager:
    ...
    def _find_adb(self):
        """Find ADB executable using universal search"""
        self.adb_path = Config.find_executable("adb")
...
<body>
        ...
        <div class="container">
            ...
            <div class="card">
                ...
                <div class="file-toolbar">
                    ...
                    <input type="file" id="upload-input" class="hidden" onchange="performUpload()">
                    <button class="btn-sm" onclick="triggerUpload()">📤 Upload</button>
    ...
    <script>
        ...
        async function performUpload() {
            ...
            const destDir = state.mode === 'mobile' ? state.mobilePath : state.pcPath;
        ...
        async function downloadFile(name, fullPath) {
            const url = `/api/download?mode=${state.mode}&path=${encodeURIComponent(fullPath)}`;
        ...
        async function loadFiles() {
            ...
            try {
                ...
                data.items.forEach(item => {
                    ...
                    let meta = `<span class="file-meta">${item.size}</span>`;
                    if (!item.is_dir) {
                        meta = `<button class="btn-sm" style="margin-left:1rem; padding: 2px 8px; font-size: 0.7rem;" onclick="event.stopPropagation(); downloadFile('${item.name}', '${item.full_path}')">⬇</button>` + meta;
                    }
...
"""
    ...
    def start(self):
        ...
        self.server = ThreadingHTTPServer(('0.0.0.0', 0), SecureHTTPHandler)
...
class SecureHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/':
            ...
            self.end_headers()
            self.wfile.write(self.server.app.HTML_TEMPLATE.encode('utf-8'))
            return

        # Auth Check for API
        auth = self.headers.get('Authorization')
        if not auth or auth != self.server.app.token:
        ...
        if parsed_path.path == '/api/stats':
            ...
            self.end_headers()
            info = self.server.app.adb.get_device_info()
            batt = self.server.app.adb.get_battery_info() or {}
        ...
        if parsed_path.path == '/api/files':
            ...
            if mode == 'mobile':
                items = self.server.app.adb.list_directory(path)
            else:
                try:
                    if path == '/' or path == "":
                        ...
                        if os.path.isdir(p):
                            for name in os.listdir(p):
                                try:
                                    ...
                                    size = '-' if is_dir else self._fmt_size(os.path.getsize(f))
                                    items.append({'name': name, 'is_dir': is_dir, 'size': size, 'full_path': f.replace('\\','/')})
                                except: pass
                        path = p.replace('\\','/')
                except: pass
        ...
        if parsed_path.path == '/api/download':
            ...
            try:
                ...
                if mode == 'mobile':
                    temp_dir = Path(os.environ.get('TEMP', './temp'))
                    temp_dir.mkdir(exist_ok=True)
                    temp_file = temp_dir / f"web_dl_{secrets.token_hex(4)}_{filename}"
                    success, err = self.server.app.adb.pull(path, temp_file)
                    if not success: raise Exception(err)
                    serve_path = temp_file
                else:
                    serve_path = Path(path)

                if serve_path.exists() and serve_path.is_file():
                    self.send_response(200)
                    self.send_header('Content-type', 'application/octet-stream')
                    self.send_header('Content-Disposition', f'attachment; filename="{filename}"')
                    self.send_header('Content-Length', serve_path.stat().st_size)
                    self.end_headers()
                    with open(serve_path, 'rb') as f:
                        shutil.copyfileobj(f, self.wfile)
                    
                    if mode == 'mobile' and temp_file:
    ...
    def do_POST(self):
        ...
        if self.path == '/api/login':
            ...
            if data.get('pin') == self.server.app.pin:
                self.send_response(200)
                self.send_header('Content-Type', 'application/json')
                self.end_headers()
                self.wfile.write(json.dumps({'success': True, 'token': self.server.app.token}).encode('utf-8'))
            else:
        ...
        if self.path == '/api/upload':
            try:
                dest = self.headers.get('X-Dest')
                mode = self.headers.get('X-Mode')
                filename = self.headers.get('X-Filename')
                
                if not all([dest, mode, filename]):
                    self.send_response(400)
                    self.end_headers()
                    self.wfile.write(b"Missing headers")
                    return

                # Read binary data
                temp_dir = Path(os.environ.get('TEMP', './temp'))
                temp_dir.mkdir(exist_ok=True)
                local_temp = temp_dir / f"upload_{secrets.token_hex(4)}_{filename}"
                
                with open(local_temp, 'wb') as f:
                    chunk_size = 64 * 1024
                    left = content_length
                    while left > 0:
                        chunk = self.rfile.read(min(left, chunk_size))
                        if not chunk: break
                        f.write(chunk)
                        left -= len(chunk)
                
                if mode == 'mobile':
                    dest_file = (Path(dest) / filename).as_posix()
                    success, err = self.server.app.adb.push(local_temp, dest_file)
                    local_temp.unlink(missing_ok=True)
                    if not success: raise Exception(err)
                else:
                    dest_path = Path(dest) / filename
                    shutil.move(str(local_temp), str(dest_path))
...
class UltimateMobileManager:
    ...
    def __init__(self):
        ...
        self.auto_mirror_var = BooleanVar(value=True)
    ...
    def _create_menu(self):
        ...
        device_menu.add_command(label="Smooth Mirror (Wi-Fi)", command=self._start_scrcpy_smooth)
    ...
    def _create_device_bar(self, parent):
        ...
        # Extra info (Mirror Controls)
        extra_frame = Frame(bar, bg=Config.BG_CARD)
        extra_frame.pack(side=RIGHT, padx=20, pady=10)
        
        # New: Auto Mirror Checkbox
        Checkbutton(extra_frame, text="[AUTO] Auto Mirror", variable=self.auto_mirror_var,
                   bg=Config.BG_CARD, fg=Config.ACCENT, selectcolor=Config.BG_SECONDARY,
                   activebackground=Config.BG_CARD, font=("Segoe UI", 9, "bold")).pack(side=LEFT, padx=10)
        
        Button(extra_frame, text="[RST] Reset Mirror", command=lambda: setattr(self, 'mirror_started', False),
               bg=Config.BG_TERTIARY, fg=Config.TEXT_DIM, relief=FLAT, font=("Segoe UI", 8)).pack(side=LEFT, padx=5)
        ...
        def fetch_and_update():
            ...
            if connected:
                ...
                def update_ui():
                    ...
                    # New: Auto Mirror Trigger
                    if self.auto_mirror_var.get() and not self.mirror_started:
                        self.mirror_started = True
                        self._start_scrcpy_smooth()
                ...
                def update_disconnected():
                    ...
                    self.mirror_started = False # Reset on disconnect
    ...
    def _start_scrcpy(self):
        """Start Screen Mirroring - Robust Mode"""
        ...
        if Config.SCRCPY_PATH.exists():
            try:
                # Prepare environment: Explicitly tell scrcpy where ADB is
                env = os.environ.copy()
                env["ADB"] = str(self.adb.adb_path)
                # Also ensure platform-tools is in PATH for any other auxiliary calls
                env["PATH"] = f"{Config.PLATFORM_TOOLS}{os.pathsep}{env.get('PATH', '')}"

                # Minimal Stable Flags: Target ID, No Audio, Stay Awake
                cmd = [str(Config.SCRCPY_PATH), '-s', self.adb.device_id, '--no-audio', '--stay-awake']
                ...
                self.status.set(f"📺 Mirroring: {self.adb.device_id}")
            except Exception as e:
    ...
    def _start_scrcpy_smooth(self):
        """Start Screen Mirroring (Smooth/Wi-Fi Optimized) - Robust Mode"""
        if not self.adb.device_id:
            # Try to auto-detect if possible
            connected, dev_id, _ = self.adb.check_connection()
            if not connected:
                messagebox.showwarning("No Device", "Please connect a device first.")
                return
        ...
        if Config.SCRCPY_PATH.exists():
            try:
                ...
                # Optimized for v2.0+: Explicit ID, Buffer 1, 8M Bitrate, 60fps
                cmd = [
                    str(Config.SCRCPY_PATH), 
                    '-s', self.adb.device_id,
                    '--video-buffer', '1', 
                    '--bit-rate', '8M', 
                    '--max-fps', '60',
                    '--no-audio', '--stay-awake'
                ...
                self.status.set(f"📺 Smooth Mirror: {self.adb.device_id}")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to start smooth mirror: {str(e)}")
        else:
            self._show_scrcpy_error()
...
def slow_print(text, delay=0.03, indent=4):
...
def slow_print_fast(text, delay=0.01, indent=4):
    sys.stdout.write(" " * indent)
    for char in text:
        sys.stdout.write(char)
        sys.stdout.flush()
...
def hacker_intro():
    ...
    for msg in messages:
        slow_print_fast(msg, 0.015, indent=4)
        time.sleep(0.2)


def print_banner_animated():
    """Print the main colorful banner with animation"""
    print(f"    {C.BYEL}[*]{C.RST} {C.BCYN}Decrypting UI Layers...{C.RST}")
    time.sleep(0.3)
    matrix_effect(1.0)
...
def print_start_message_animated():
    ...
    print(f"    {C.BGRN}[SYSTEM]{C.RST} {C.BWHT}All Cores Initialized Successfully!{C.RST}")
    print(f"    {C.BGRN}[TIME]{C.RST}   {C.BYEL}{current_time}{C.RST}")
    print(f"    {C.BCYN}[LAUNCH]{C.RST} {C.BCYN}Deploying Ultimate GUI Interface...{C.RST}")
    print()
    
    border = "=" * 61
    print(f"    {C.BMAG}{border}{C.RST}")
    print(f"    {C.BWHT}   Press {C.BRED}Ctrl+C{C.BWHT} to exit | {C.BGRN}Happy Managing!{C.RST}")
    print(f"    {C.BMAG}{border}{C.RST}")
    print()
