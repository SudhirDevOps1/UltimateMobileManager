            ...
            if mode == 'mobile':
                ...
                try:
                    ...
                    if path == '/' or path == "":
                        ...
                        for d in drives:
                            items.append({'name': d, 'is_dir': True, 'size': 'Drive', 'full_path': d})
                        path = "/"
                    else:
...
class UltimateMobileManager:
    ...
    def _create_ui(self):
        ...
        Button(btn_frame, text="✅ Connect USB", command=self._refresh_all, **c_btn_cfg).pack(side=LEFT, padx=2)
        Button(btn_frame, text="📡 Connect Wi-Fi", command=self._show_wireless_dialog, **c_btn_cfg).pack(side=LEFT, padx=2)
        Button(btn_frame, text="🚫 Disconnect All", command=self._disconnect_wireless, **d_btn_cfg).pack(side=LEFT, padx=2)
